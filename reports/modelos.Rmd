...
nyc_sales %>% 
  ...
  geom_point(aes(log(GROSS_SQUARE_FEET), log(SALE_PRICE), color = BUILDING_CLASS_CATEGORY), 
             alpha = 0.6, size = 0.7) +
  facet_wrap(~Neighborhood2, scales = 'free') + theme(legend.position="bottom")

nyc_sales %>% 
  ggplot() +
  geom_point(aes(log(GROSS_SQUARE_FEET), log(SALE_PRICE), color = BUILDING_CLASS_CATEGORY), 
             alpha = 0.6, size = 0.7) +
  facet_wrap(~Borough, scales = 'free') + theme(legend.position="bottom")

nyc_sales %>% 
  ggplot() +
  geom_point(aes(log(GROSS_SQUARE_FEET), log(SALE_PRICE), color = BUILDING_CLASS_CATEGORY), 
             alpha = 0.6, size = 0.7) +
  facet_grid(BUILDING_CLASS_CATEGORY~Borough, scales = 'free') + theme(legend.position="bottom")
